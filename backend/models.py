...
class DiarizationRequest(BaseModel):
    ...
    num_speakers: int | None = Field(None, ge=1, le=20)
...
class TranslationRequest(BaseModel):
    ...
    min_chars: int | None = Field(None, ge=0)
    tone: str = "natural"
...
class DubbingProjectCreate(BaseModel):
...
class DubbingSpeakerResponse(BaseModel):
    """A speaker mapped to a voice profile."""
...
class DubbingProjectResponse(BaseModel):
    ...
    segment_count: int = 0
    speakers: list[DubbingSpeakerResponse] = []
    created_at: datetime | None = None


class DubbingSegmentResponse(BaseModel):
    """A dubbing timeline segment."""
...
class DubbingSegmentUpdate(BaseModel):
    """Update fields on a dubbing segment."""

    translated_text: str | None = None
    alignment: str | None = Field(None, pattern="^(start|center|end)$")
    pace_multiplier: float | None = Field(None, ge=0.5, le=2.0)
