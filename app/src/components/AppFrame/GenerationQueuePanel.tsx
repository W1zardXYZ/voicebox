 ...
 * it is visible on every route while clips are queued or generating: lists each
 * active generation with its text preview, state, and a per-item progress bar
 * (indeterminate while loading, % while sampling), plus a total aggregate.
 */
export function GenerationQueuePanel() {
  ...
  if (items.length === 0 && !hasLiveProgress) {
    return null;
  }

  return (
    <div className="fixed bottom-4 right-4 z-50 w-80 max-w-[calc(100vw-2rem)] rounded-lg border bg-background/95 backdrop-blur shadow-lg">
      <div className="flex items-center justify-between px-3 py-2 border-b">
        ...
        >
          ...
          {avg != null && <span className="text-accent">· {avg}%</span>}
      ...
      {open && (
        <div className={cn('px-3 py-2 max-h-64 overflow-y-auto')}>
          {items.map((item) => (
